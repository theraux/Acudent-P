function initStaffAssistantAppointments() {
    ...
    function renderStaffAssistantCalendar() {
                    ...
                    box.addEventListener('click', () => {
                        ...
                        if (datePickingMode) {
                            ...
                            if (appointmentMode === 'add') {
                                openStaffAssistantAppointmentModal(selectedDate);
                            } else if (appointmentMode === 'edit') {
                                openStaffAssistantEditModal(selectedDate);
                            }
                            datePickingMode = false;  // Reset after use
                            appointmentMode = null;   // Reset mode
                        } else {
                            if (dayAppointments.length > 0) {
                                openStaffAssistantViewModal(new Date(year, month, day), dayAppointments);
                            } else {
    ...
    function StaffAssistantprevmonth() {
        currentDate.setMonth(currentDate.getMonth() - 1);
        renderStaffAssistantCalendar();
    }

    function StaffAssistantnextmonth() {
        currentDate.setMonth(currentDate.getMonth() + 1);
        renderStaffAssistantCalendar();
    }

    window.StaffAssistantprevmonth = StaffAssistantprevmonth;
    window.StaffAssistantnextmonth = StaffAssistantnextmonth;
    ...
    function openStaffAssistantAppointmentModal(date) {
    ...
    function openStaffAssistantEditModal(date) {
     ...
     function openStaffAssistantViewModal(date, dayAppointments) {
...
document.addEventListener("DOMContentLoaded", initStaffAssistantAppointments)
function initAdminStaffManagement() {
    const page = document.querySelector('#admin-staff-page-id');
  if (!page) return;
    ...
    // ===================== STAFF BOX CLICK (TILE VIEW) ===================== //
    const staffBoxes = document.querySelectorAll('.profile-box');
    staffBoxes.forEach(box => {
        box.addEventListener('click', () => {
            const url = box.getAttribute('data-target');
            const staffId = box.getAttribute('data-id'); 
            if (!url) return;

            const urlWithParam = staffId
                ? `${url}?id=${encodeURIComponent(staffId)}`
                : url;
    ...
    viewButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const staffId = btn.closest('tr')?.querySelector('td:nth-child(2)')?.textContent?.trim();
...
document.addEventListener('DOMContentLoaded', initAdminStaffManagement);
